...
{
    public partial class CardRegistrationOAuthExample : ContentPage
    {
        ...
        public CardRegistrationOAuthExample()
        {
        ...
        public async Task StartWebViewFlow()
        {
            ...
            var formFields = new Dictionary<string, object>
            {
                ...
                { "Detail", "{\"Payments\":[{\"Items\":[{\"Description\":\"test\",\"Quantity\":\"1\",\"Amount\":" + amount + ",\"Tax\":0}],\"MerchantKey\":\"TEST-001\",\"Service\":\"" + orderId + "\",\"MerchantName\":\"Oriental Bank\",\"Description\":\"test\",\"Amount\":" + amount + ",\"Tax\":0,\"Currency\":\"840\"}]}" },
            ...
            var authHash = await GetPaymentOAuthToken(_config.ClientId, _config.ClientSecret, _config.UserId, orderId, amount);
            formFields.Add("Token", authHash);

            // At the moment this fields needs to be added in order to complete the registration flow. This might be revised in future updates.
            formFields.Add("Authentication", "");
        ...
        /// <returns></returns>
        private async Task<string> GetPaymentOAuthToken(string clientId, string clientSecret, string userId, string orderId, decimal amount)
        {
            ...
            HttpResponseMessage response = await httpClient.PostAsync("https://stage.agilpay.net/PaynetWebApiTEST/oauth/paymenttoken", content);
